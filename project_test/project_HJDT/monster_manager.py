...
def check_monster(view_manager, player):
	...
	for monster in monster_list.sprites():
		...
		if monster.type == TYPE_BOMB:
			# 角色被炸弹炸到
			if player.is_hurt(monster.x, monster.end_x, monster.start_y, monster.end_y):
				# 将怪物设置为死亡状态
				monster.is_die = True
		...
		# 将del_bullet_list中包含的所有子弹从bullet_list中删除
		bullet_list.remove(del_bullet_list)
		# 检测怪物的子弹是否打中了角色
		monster.check_bullet(player)
	# 将已经死亡的怪物（保存在del_list列表中）添加到die_monster_list中
	die_monster_list.add(del_list)
	# 将已经死亡的怪物（保存早del_list列表中）从monster_list列表中删除
	monster_list.remove(del_list)
